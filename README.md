...
_Note:_ The same approach can be used with other CLIs; e.g. `claude`, `amp`, `opencode`, etc. The reference loop in this repo now uses Codex by default.

_What controls task continuation?_

The continuation mechanism is elegantly simple:

1. _Bash loop runs_ → feeds `PROMPT.md` to Codex CLI (by default)
...
The reference `files/loop.sh` in this repo is now Codex-first and adds fast-mode and thinking-effort toggles on top of the usual Ralph loop behavior.
...
_Reference loop behavior:_

- Defaults to Codex CLI; `RALPH_CLI=claude` remains available as an explicit override
- `--fast` enables Codex `fast_mode`; without it the script explicitly disables fast mode
- `--effort low|medium|high|xhigh` sets Codex `model_reasoning_effort` for that run
- Stops cleanly when an iteration produces no new commit instead of spinning forever
- Pushes after each successful committed iteration, creating the remote branch if needed
...
- `--fast` keyword → Enables Codex fast mode for that run
- `--effort low|medium|high|xhigh` → Sets Codex reasoning effort for that run
...
_Core function:_ Continuously feeds the selected prompt file to Codex by default, manages iteration limits, and pushes changes after each task completion.
